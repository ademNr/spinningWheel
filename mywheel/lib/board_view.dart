...
class BoardView extends StatefulWidget {
  ...
  BoardView({ required this.angle, required this.current, required this.items}) ;
...
class _BoardViewState extends State<BoardView> {
  Size get size => Size(MediaQuery.of(context).size.width * 1,
      ...
      children: <Widget>[
        ...
        Transform.rotate(
          ...
          child: Stack(
            alignment: Alignment.center,
            children: <Widget>[
              for (var luck in widget.items) ...[_buildCard(luck as Luck)],
              for (var luck in widget.items) ...[_buildImage(luck as Luck)],
            ],
          ),
        ),
        Container(
    ...
    return Transform.rotate(
      ...
      child: Container(
        ...
        child: ConstrainedBox(
          constraints:
              BoxConstraints.expand(height: size.height / 2, width: 100),
          child: Center(child: Text("${luck.asset}",style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold, color: Colors.white),)),
        ),
      ),
    );
  }
